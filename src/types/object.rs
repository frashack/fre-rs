...
pub unsafe trait AsObject<'a>: Sized + Copy + Eq + Display + Into<FREObject> + Into<Object<'a>> {
    ...
    fn get_type(self) -> Type {
        if self.is_null() {return Type::Null;}
    ...
    fn get_property (self, name: UCStr) -> Result<Object<'a>, ExternalError<'a>> {
        ...
        let r = unsafe {FREGetObjectProperty(self.as_ptr(), name.as_ptr(), &mut obj, &mut thrown)};
...
impl<'a, O: AsObject<'a>> From<O> for UCStr {
...
impl<'a> Object<'a> {
    pub const NULL: Object<'static> = Object(std::ptr::null_mut(), PhantomData);
    #[allow(unused_variables)]
    pub fn new (frt: &FlashRuntime<'a>, class: UCStr, args: Option<&[Object]>) -> Result<Object<'a>, ExternalError<'a>> {
        let args = args.unwrap_or_default();
        debug_assert!(args.len() <= u32::MAX as usize);
        let mut obj = std::ptr::null_mut();
        let mut thrown = std::ptr::null_mut();
        let r = unsafe {FRENewObject(class.as_ptr(), args.len() as u32, transmute(args.as_ptr()), &mut obj, &mut thrown)};
        if let Ok(e) = ExternalError::try_from(r, Some(unsafe {transmute(thrown)})) {
            Err(e)
        }else{
            assert!(!obj.is_null());
...
impl TryFrom<Object<'_>> for String {
    type Error = FfiError;
    fn try_from(value: Object) -> Result<Self, Self::Error> {
...
impl<'a> TryFrom<Object<'a>> for Context<'a> {
    ...
    fn try_from(value: Object) -> Result<Self, Self::Error> {
        ...
        let r = unsafe {FREGetFREContextFromExtensionContext(value.0, &mut ctx)};
        if let Ok(e) = r.try_into() {
            Err(e)
        } else {Ok(unsafe {transmute(ctx)})}
    }
}
...
impl From<()> for Object<'static> {fn from(_: ()) -> Self {null}}
